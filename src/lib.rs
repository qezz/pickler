...
pub enum PickleValue<'a> {
    ...
    String(&'a str),
    Bytes(&'a [u8]),
    List(std::sync::Arc<Vec<PickleValue<'a>>>),
    Tuple(std::sync::Arc<Vec<PickleValue<'a>>>),
    Dict(std::sync::Arc<Vec<(PickleValue<'a>, PickleValue<'a>)>>),
    Set(std::sync::Arc<Vec<PickleValue<'a>>>),
    FrozenSet(std::sync::Arc<Vec<PickleValue<'a>>>),
    Object {
        module: &'a str,
        attr: &'a str,
        args: std::sync::Arc<Vec<PickleValue<'a>>>,
        state: std::sync::Arc<PickleValue<'a>>,
    },
    Global {
        module: &'a str,
        attr: &'a str,
